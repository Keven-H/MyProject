...
+(NSNumber *)sizeOfDirector:(NSString *)folderPath
{
    ...
    while ((fileName = [childFilesEnumerator nextObject]) != nil){
        ...
        folderSize += [self sizeOfItemAtPath:fileAbsolutePath].floatValue;
    }
    return [NSNumber numberWithLongLong:folderSize];
}
...
+(BOOL)writeFileAtPath:(NSString *)path content:(NSObject *)content error:(NSError **)error
{
    ...
    //    {
    //        [UIImageJPEGRepresentation((UIImage *)content, 0.75) writeToFile:absolutePath atomically:YES];
    //
    ...
    //
    //    return YES;
    return [self writeFileAtPath:path content:content error:error isPNG:NO];
}


+(BOOL)writeFileAtPath:(NSString *)path content:(NSObject *)content error:(NSError **)error isPNG:(BOOL)ispng
{
    if(content == nil)
    {
        //        [NSException raise:@"Invalid content" format:@"content can't be nil."];

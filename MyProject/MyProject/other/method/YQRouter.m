...
//  YQRouter
//
//  Created by liyunqi on 22/08/2017.
...
static NSString * const YQ_ROUTER_WILDCARD_CHARACTER = @"~";
static NSString *specialCharacters = @"/?&.";

NSString *const YQRouterParameterURL = @"YQRouterParameterURL";
NSString *const YQRouterParameterCompletion = @"YQRouterParameterCompletion";
NSString *const YQRouterParameterUserInfo = @"YQRouterParameterUserInfo";
@interface YQRouter()
/**
 *  保存了所有已注册的 URL
...
@implementation YQRouter

+ (instancetype)sharedInstance
{
    static YQRouter *instance = nil;
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion
{
    ...
    if (parameters) {
        YQRouterHandler handler = parameters[@"block"];
        if (completion) {
            parameters[YQRouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[YQRouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
{
    YQRouter *router = [YQRouter sharedInstance];
    
    URL = [URL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSMutableDictionary *parameters = [router extractParametersFromURL:URL];
    YQRouterObjectHandler handler = parameters[@"block"];
...
+ (void)registerURLPattern:(NSString *)URLPattern toObjectHandler:(YQRouterObjectHandler)handler
{
    [[self sharedInstance] addURLPattern:URLPattern andObjectHandler:handler];
}

- (void)addURLPattern:(NSString *)URLPattern andHandler:(YQRouterHandler)handler
{
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(YQRouterObjectHandler)handler
{
    NSMutableDictionary *subRoutes = [self addURLPattern:URLPattern];
    if (handler && subRoutes) {
        subRoutes[@"_"] = [handler copy];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url
{
    ...
    parameters[YQRouterParameterURL] = url;
    ...
    for (NSString* pathComponent in pathComponents) {
        ...
        for (NSString* key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:YQ_ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
...
- (NSArray*)pathComponentsFromURL:(NSString*)URL
{
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        NSString *headPath=pathSegments[0];
        [pathComponents addObject:headPath.lowercaseString];
        
        // 如果只有协议，那么放一个占位符
         URL = pathSegments.lastObject;
        if (pathSegments.count>2) {
            URL=[pathSegments objectAtIndex:1];
        }
       
        if (!URL.length) {
            [pathComponents addObject:YQ_ROUTER_WILDCARD_CHARACTER];
        }
    }
